import { DIAS, CATEGORIAS, PRIMEIRO_DIA, datasDaSemana } from "./semana";
import {
  OM,
  MESES,
  DIAS_EXTENSO,
...
// =====================================================================
// Quem entra na planilha
// =====================================================================

// A relacao nominal do Rancho traz SEMPRE todo o efetivo — o que muda de
// um dia para o outro sao só os X. Por isso a planilha e montada a partir
// da relacao de autorizados, e nao da lista de quem criou acesso no site:
// quem nunca entrou continua tendo a linha dele, em branco.
//
// As marcacoes chegam indexadas pelo id do militar; aqui elas sao
// reindexadas pelo identificador, que e o que liga as duas listas.
export function efetivoParaPlanilha(autorizados, militares, marcacoes) {
  // Sem a relacao (nao carregou, ou banco antigo), usa quem tem acesso —
  // e melhor sair incompleta do que nao sair.
  if (!autorizados?.length) {
    return { pessoas: militares, marcas: marcacoes };
  }

  const porIdentificador = new Map(
    militares.filter((m) => m.identificador).map((m) => [m.identificador, m])
  );

  const pessoas = autorizados.map((a) => ({ ...a, id: a.identificador }));

  const marcas = {};
  for (const a of autorizados) {
    const militar = porIdentificador.get(a.identificador);
    const marcado = militar && marcacoes[militar.id];
    if (marcado) marcas[a.identificador] = marcado;
  }

  return { pessoas, marcas };
}
...
// O formulario oficial usa borda "hair" (o tracinho mais fino do Excel) em
// toda a grade — e o que da aquele aspecto leve. "thin" ja fica pesado.
const FINA = { style: "hair", color: { argb: PRETO } };
// A linha de assinatura precisa ser visivel; "hair" some na impressao.
const TRACO = { style: "thin", color: { argb: PRETO } };
...
function escrever(aba, linha, coluna, valor, opcoes = {}) {
  ...
  // Praticamente tudo no formulario e Times New Roman negrito 9; o peso
  // normal e a excecao (marcas X e as linhas inutilizadas), pedido por `leve`.
  c.font = {
    name: FONTE,
    size: opcoes.tamanho || 9,
    bold: !opcoes.leve,
    italic: !!opcoes.italico,
    underline: !!opcoes.sublinhado,
  };
  if (opcoes.borda !== false) {
    c.border = { top: FINA, left: FINA, bottom: FINA, right: FINA };
  } else if (opcoes.bordaBaixo) {
    c.border = { bottom: TRACO };
  }
  return c;
}
...
function montarCabecalho(aba, diaKey, data) {
  ...
  faixa(aba, 2, 1, 3, 4, OM.secao, { negrito: true });
  faixa(aba, 2, 5, 2, COL.ultima, OM.companhia, { negrito: true });
  faixa(aba, 3, 5, 3, COL.ultima, undefined, { borda: false });

  faixa(aba, 4, 1, 4, 4, "=", { borda: false });
  faixa(aba, 4, 5, 4, 15, DIAS_EXTENSO[diaKey], { negrito: true });
  escrever(aba, 4, 16, String(data.getDate()).padStart(2, "0"), { negrito: true });
  faixa(aba, 4, 17, 4, 19, mesCapitalizado(data), { negrito: true });
  faixa(aba, 4, 20, 4, 21, data.getFullYear(), { negrito: true });
}
...
function montarQuadroEtapas(aba, totais) {
  ...
  faixa(aba, 5, 1, 6, 4, "ETAPAS  REDUZIDAS", { negrito: true });
  ...
  faixa(aba, 5, 8, 6, 13, "ETAPAS COMPLETAS", { negrito: true });
  faixa(aba, 5, 14, 5, 16, "À ALIMENTAR", { negrito: true });
  escrever(aba, 6, 14, "SOMA", { negrito: true });
  escrever(aba, 6, 15, "OUTRA OM", { negrito: true, vertical: true });
  escrever(aba, 6, 16, undefined);

  faixa(aba, 5, 17, 5, 18, "QTATIVOS", { negrito: true });
  escrever(aba, 6, 17, "TIPO", { negrito: true, vertical: true });
  escrever(aba, 6, 18, "QUANTIDADE", { negrito: true, vertical: true });

  faixa(aba, 5, 19, 5, 21, "COMPLEMENTOS", { negrito: true });
  escrever(aba, 6, 19, undefined);
  escrever(aba, 6, 20, "C ESC", { negrito: true, vertical: true });
  escrever(aba, 6, 21, "CF 60 %", { negrito: true, vertical: true });
  ...
  linhas.forEach((l, i) => {
    const linha = 7 + i;
    faixa(aba, linha, 1, linha, 4, rotulos[i], { negrito: true, alinhamento: ESQUERDA });
    escrever(aba, linha, 5, dois(l.cafe));
    escrever(aba, linha, 6, dois(l.almoco));
    escrever(aba, linha, 7, dois(l.janta));

    faixa(aba, linha, 8, linha, 13, rotulosCompletas[i], { negrito: true, alinhamento: ESQUERDA });
  ...
  faixa(aba, 10, 1, 10, 4, "SOMA", { negrito: true, alinhamento: ESQUERDA });
...
function montarCabecalhoRelacao(aba) {
  ...
  [COL.marcas, COL.epMarcas, ...COL.ev.map((b) => b.marcas)].forEach((cols) => {
    cols.forEach((c, i) => escrever(aba, l2, c, letras[i], { tamanho: 10 }));
  });
}
...
function montarRelacao(aba, militares, marcacoes, diaKey) {
  ...
  for (let i = 0; i < altura; i++) {
    ...
    if (item?.subtitulo) {
      ...
      escrever(aba, linha, COL.marcas[0], xis(marca, "cafe"), { leve: true });
      escrever(aba, linha, COL.marcas[1], xis(marca, "almoco"), { leve: true });
      escrever(aba, linha, COL.marcas[2], xis(marca, "janta"), { leve: true });
    } else {
    ...
    if (ep) {
      ...
      escrever(aba, linha, COL.epNumero, ep.numero_guerra, { tamanho: 10.5 });
      escrever(aba, linha, COL.epNome, String(ep.nome || "").trim(), {
        italico: true,
        tamanho: 8,
      });
      escrever(aba, linha, COL.epMarcas[0], xis(marca, "cafe"), { leve: true });
      escrever(aba, linha, COL.epMarcas[1], xis(marca, "almoco"), { leve: true });
      escrever(aba, linha, COL.epMarcas[2], xis(marca, "janta"), { leve: true });
    } else {
    ...
    COL.ev.forEach((bloco, b) => {
      ...
      if (ev) {
        ...
        escrever(aba, linha, bloco.numero, ev.numero_guerra, { tamanho: 10.5 });
        escrever(aba, linha, bloco.marcas[0], xis(marca, "cafe"), { leve: true });
        escrever(aba, linha, bloco.marcas[1], xis(marca, "almoco"), { leve: true });
        escrever(aba, linha, bloco.marcas[2], xis(marca, "janta"), { leve: true });
      } else {
        // Linha sem militar: inutilizada, como no formulario em papel.
        escrever(aba, linha, bloco.numero, "///////", { leve: true });
...
function montarAba(pasta, dia, data, militares, marcacoes) {
  const aba = pasta.addWorksheet(dia.label.slice(0, 3).toUpperCase(), {
    pageSetup: {
      ...
      fitToWidth: 1,
      fitToHeight: 1,
      // Margens do proprio formulario oficial.
      margins: {
        left: 0.1035,
        right: 0.0722,
        top: 0.1965,
...
// ---------------------------------------------------------------------
// Monta a pasta de trabalho com as sete abas. Fica separada do download
// para que a planilha possa ser gerada tambem fora do navegador — e assim
// dar para conferir o resultado sem depender de alguem clicar no botao.
export function montarPasta(ExcelJS, militares, marcacoes, inicio, escolhidos) {
  const datas = datasDaSemana(inicio);
  // Sem escolha, vai o periodo inteiro (uma aba por dia). A seção tambem
  // baixa dia a dia, porque e assim que entrega ao Rancho.
  const dias = escolhidos?.length
    ? DIAS.filter((d) => escolhidos.includes(d.key))
    : DIAS;
  ...
  return { pasta, datas, dias };
}

export async function baixarExcel(militares, marcacoes, inicio, escolhidos) {
  const ExcelJS = await carregarExcelJS();
  const { pasta, datas, dias } = montarPasta(
    ExcelJS,
    militares,
    marcacoes,
    inicio,
  ...
  link.href = url;
  // Um dia só leva o nome do dia; o período inteiro leva a data de abertura.
  const nome =
    dias.length === 1
      ? `arranchamento_${dias[0].key}_${datas[dias[0].key].curta.replace("/", "-")}`
      : `arranchamento_${datas[PRIMEIRO_DIA].curta.replace("/", "-")}`;
