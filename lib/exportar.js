...
function escrever(aba, linha, coluna, valor, opcoes = {}) {
  ...
  c.alignment = opcoes.alinhamento || CENTRO;
  c.font = {
    name: "Arial",
    size: opcoes.tamanho || 8,
    bold: !!opcoes.negrito,
  };
...
function montarQuadroEtapas(aba, totais) {
  ...
  faixa(aba, 5, 1, 6, 4, "ETAPAS REDUZIDAS", { negrito: true, tamanho: 9 });
  faixa(aba, 5, 5, 6, 5, "CAFÉ", { negrito: true });
  faixa(aba, 5, 6, 6, 6, "ALMOÇO", { negrito: true });
  faixa(aba, 5, 7, 6, 7, "JANTAR", { negrito: true });

  faixa(aba, 5, 8, 6, 13, "ETAPAS COMPLETAS", { negrito: true, tamanho: 9 });
  faixa(aba, 5, 14, 5, 16, "À ALIMENTAR", { negrito: true });
  escrever(aba, 6, 14, "SOMA", { negrito: true });
  escrever(aba, 6, 15, "OUTRA OM", { negrito: true });
  escrever(aba, 6, 16, undefined);

  faixa(aba, 5, 17, 5, 18, "QTATIVOS", { negrito: true });
  escrever(aba, 6, 17, "TIPO", { negrito: true });
  escrever(aba, 6, 18, "QUANTIDADE", { negrito: true });

  faixa(aba, 5, 19, 5, 21, "COMPLEMENTOS", { negrito: true });
  escrever(aba, 6, 19, undefined);
  escrever(aba, 6, 20, "C ESC", { negrito: true });
  escrever(aba, 6, 21, "CF 60 %", { negrito: true });
  ...
  linhas.forEach((l, i) => {
    const linha = 7 + i;
    faixa(aba, linha, 1, linha, 4, rotulos[i], { negrito: true });
    escrever(aba, linha, 5, l.cafe);
    escrever(aba, linha, 6, l.almoco);
    escrever(aba, linha, 7, l.janta);

    faixa(aba, linha, 8, linha, 13, rotulosCompletas[i], { negrito: true });
    escrever(aba, linha, 14, l.completa);

    // Colunas de efetivo e complementos: preenchidas a mao pela seção.
    for (let c = 15; c <= 21; c++) escrever(aba, linha, c, undefined);
  });

  faixa(aba, 10, 1, 10, 4, "SOMA", { negrito: true });
  escrever(aba, 10, 5, soma.cafe, { negrito: true });
  escrever(aba, 10, 6, soma.almoco, { negrito: true });
  escrever(aba, 10, 7, soma.janta, { negrito: true });
  faixa(aba, 10, 8, 10, 13, undefined);
  for (let c = 14; c <= 21; c++) escrever(aba, 10, c, undefined);

  faixa(aba, 11, 1, 11, 13, undefined, { borda: false });
  escrever(aba, 11, 14, soma.completa, { negrito: true });
...
function montarAssinaturas(aba, data) {
  faixa(aba, 12, 9, 13, 12, "____________________________", { borda: false });
  faixa(aba, 12, 15, 13, 21, "____________________________________", { borda: false });
...
function montarCabecalhoRelacao(aba) {
  ...
  faixa(aba, l1, COL.epNumero, l2, COL.epNome, "SD EP", opc);
