...
// Terca-feira que abre o proximo periodo de arranchamento.
//
// Como o periodo vai de terca a segunda, e sempre a PROXIMA terça: numa
// segunda-feira, o periodo que se preenche comeca no dia seguinte — que e
// exatamente o que a seção faz hoje com o papel. Na terca ja vira para a
// terca seguinte, porque a daquele dia ja esta em curso.
export function inicioPeriodo(data = new Date()) {
  const d = new Date(data);
  d.setHours(0, 0, 0, 0);
  const faltam = (2 - d.getDay() + 7) % 7 || 7; // 2 = terça
