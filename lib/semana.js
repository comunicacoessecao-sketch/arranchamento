...
// Como o militar aparece NO SITE (nao na planilha do Rancho, que tem
// formato proprio — ver rotuloRelacao em lib/modelo.js).
//
// Leva sempre o numero de guerra junto quando existe: sem ele, os soldados
// viravam todos "SD" nas listas do painel, impossiveis de identificar.
//
//   CAP SIQUEIRA          oficiais e graduados nao tem numero
//   CB 106 CARVALHO
//   SD 231 LEONAN
//   SD 409 SUTIL
//   SD 442                soldado ainda sem nome na relacao
export function nomeExibicao(militar) {
  if (!militar) return "";
  const partes = [militar.posto_grad, militar.numero_guerra, militar.nome]
    .map((p) => (p === null || p === undefined ? "" : String(p).trim()))
    .filter(Boolean);
  return partes.join(" ") || militar.identificador || "";
}
